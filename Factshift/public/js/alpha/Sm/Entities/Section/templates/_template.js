    ...
    Sm.Entities.Section.templates._template = {
        body_outer:    {
            full: function (Entity) {
                ...
                var template = [
                    '<section title="<%- subtitle %>" class="factshift-section factshift-entity  <% if( ! has_title) {%>no-title<% } %> ' + subtype + '" data-entity_type="Section" data-ent_id="<%- ent_id %>" data-id="<%- id %>" data-title=\'<%- title %>\'>',
                    '__BUTTON_CONTROL__',
                    '__CONTENT__',
                    '</section>'
                ];
