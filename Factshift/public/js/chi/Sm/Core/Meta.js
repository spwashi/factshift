...
define(['Emitter', 'Sm', 'jquery', 'underscore', 'inflection'], function (Emitter, Sm, $) {
	...
	var Meta = Emitter.extend(
	{
		 ...
		 * @param {string|Sm.Core.MvCombo=}type
		 * @return {*}
		 */
		...
		get_datatype_of:                   function (attribute) {
			...
			if (/((h|w)a|i|adopt)s_|can/.test(attribute)) {
				...
				return "entity";
			} else if (/(_(sub)?type)|role/.test(attribute)) {
				return "enum";
			}
			return "short";
		},
		get_attribute_enum_object:         function (attribute) {
			...
			if (this.type && this.type.toLowerCase() === split[0]) {
				split.shift();
				return this[split.join('_') + 's'];
			}
			if (this[attribute + '_obj']) return this[attribute + '_obj'];
		...
		get_type:                          function (identifier, what_to_get) {
			...
			for (var t in types) {
				...
				if (t == identifier || types[t].id == identifier) {
					switch (what_to_get) {
						case 'index':
							return t;
						case 'id':
...
//
		...
		set_constants:                     function (constants) {
			constants = constants || {};
			var type  = this.type;
			if (this.type) {
				var toLower = type.toLowerCase() + '_';
				for (var c_name in constants) {
					if (!constants.hasOwnProperty(c_name)) continue;
					if (c_name.indexOf(toLower) !== -1) this[c_name.replace(toLower, '')] = constants[c_name];
				}
			} else {
				this.constants = this.constants || {};
				for (var const_name in constants) {
					if (!constants.hasOwnProperty(const_name)) continue;
					this.constants[const_name] = constants[const_name];
				}
			}
		}
	});
	 ...
	 */
	Sm.Core.Meta = new Meta({type: false});
