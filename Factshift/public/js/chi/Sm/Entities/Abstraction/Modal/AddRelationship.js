	...
	Sm.loaded.when_loaded(['Extras_Modal'], function () {
		...
		Sm.Entities.Abstraction.Modal.AddRelationship = Sm.Extras.Modal.extend(
		{
			init:               function (settings) {
				settings                = settings || {};
				this.relationship_index = settings.relationship_index || false;
				settings.data           =
			...
			generate_element:   function (config) {
				if (this.EntityArr.length > 0 && this.EntityArr.length <= 2) {
					...
					var rel_inds             = (config.relationship_index || this.relationship_index) ? [config.relationship_index || this.relationship_index] : (this.EntityArr[1] ? Meta.get_possible_relationship_indices(this.EntityArr[1] || false) : false);
					var relationship_indices = Meta.get_named_relationship_indices({relationship_indices: rel_inds});
					var count                = 0;
					for (var r_i in relationship_indices) {
						if (!relationship_indices.hasOwnProperty(r_i)) continue;
						count++;
					}
					if (count === 1) relationship_indices._only_one = r_i;
			...
			on_open:            function () {
				...
				if (this.relationship_index) {
					var $select = this.get_content_element(true);
					Sm.CONFIG.DEBUG && console.log($select[0].innerHTML);
					var $r = $select.find('[value="' + this.relationship_index + '"]');
					Sm.CONFIG.DEBUG && console.log($r, this.relationship_index);
					this.on_select(this.relationship_index, {}, $r[0]);
				}
			},
