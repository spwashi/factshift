		...
		Sm.Extras.Modal             = Sm.Extras.ViewAid.extend(
		{
			...
			init:                 function (settings) {
				try {
					...
					var EntityArr          = settings.MvCombo || settings.EntityArr || false;
					if (!!EntityArr && EntityArr.constructor !== Array) EntityArr = [EntityArr];
					this.EntityArr = EntityArr || false;
					this.self_type = this.getEntityType(this.EntityArr);
			...
			getEntityType:        function (EntityArr) {
				var alternative_type;
				if (Array.isArray(EntityArr)) {
					if (EntityArr.length === 1) alternative_type = EntityArr[0].type;
				} else {
					alternative_type = EntityArr.type;
				}
