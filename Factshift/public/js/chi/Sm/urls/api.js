			...
			try {
				...
				if (settings.Relationship) {
					...
					for (var index in map_links) {
						...
						var identifier = Identity.id || Identity.ent_id || false;
						if (Identity.type && identifier) unjoined_url.push(Identity.type + '/' + identifier);
						else {
							has_failed = true;
							break;
						}
					}
					return !has_failed ? BASE_API_URL + unjoined_url.join('/') : false;
				}
				if (settings.url) {
