...
class Router extends Abstraction\Router {
    ...
    public function __construct(App $app = null) { if (isset($app)) $this->setApp($app); }
...
#########################################################
    ...
    public function match($http_method, $uri) {
        ...
        if (!empty($matches)) {
            ...
            if (($app_name !== null && $app_name !== $App->name) || $is_just_load_app) {
                #boot the app to get the routing rules that may lie in that application
                $App = App::init($app_name)->boot();
                if ($App->has_been_booted() && !in_array(implode('', $route->pattern), static::$called_route_paths)) {
                    $App->set_as_main();
                } else {
                ...
                return $App->IoC->router->match($http_method, $App->change_uri($uri));
            }
            $route->parameters = [ ];
    ...
    public function generate_url($name, $arguments = [ ], $query_arr = [ ], $default = null) {
        ...
        if ($route->path) {
            ...
            $url = trim($url, "/\n\0\x0B\t\r");
            $url = $url . '/' . (!empty($query_arr) ? (is_string($query_arr) ? $query_arr : '?' . http_build_query($query_arr)) : '');
        }
        return $url;
    }
    public function register($array) {
        ...
        foreach ($array as $index => $route) {
            ...
            if ($callback === null && !isset($route['path'])) continue;
            ...
            if ($pattern === null && !isset($route['path'])) $pattern = '{variable}:[a-zA-Z\d]*';
            $settings = [
...
#########################################################
    ...
    private function add($type, $pattern, $callback, $settings = [ ]) {
        ...
        $added_route->path                     = $settings['path'] ?? null;
        ...
        if ($added_route->route_name) {
            $this->named_routes[ $added_route->route_name ] = $added_route;
            if (isset($added_route->path)) return;
        }
