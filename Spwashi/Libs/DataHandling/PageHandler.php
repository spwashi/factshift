...
class PageHandler extends UpdatedForm {
    public $model;
    public function __construct($model = null, $change_to_default = false, $allowed_values = null) {
        ...
        $this->allowed_values = is_array($allowed_values) ? $allowed_values : Page::$api_settable_properties;
    }

    public static function validate_title(&$form_value, $data = null, $change_to_default = false, $is_subtitle = false) {
        $validity = PageValidator::getTitleValidity($form_value, $is_subtitle);
    ...
    public static function validate_password_verify(&$arr_values, $data = null, $change_to_default = false, $form_values) {
        if (isset($form_values) && isset($form_values['password'])) {
            if ($form_values['password'] === $arr_values) return static::SUCCESS;
        }
        return false;
    }
    public static function validate_description(&$form_value, $data = null, $change_to_default = false) {
        $validity = PageValidator::getDescriptionValidity($form_value);

        $max_content_length = PageValidator::MAX_DESCRIPTION_LENGTH;
        $messages           = [
            Validator::E_TOO_LONG => "Page description can only be up to {$max_content_length} characters long",
        ];
