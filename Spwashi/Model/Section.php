...
#
	...
	public static function findUsages($settings = []) {
		...
		$settings_arr = ['is_reciprocal' => true];
	 ...
	 * @return mixed
	 */
	public static function &section_find_holder_hook($self, &$properties_of_map, &$object_properties, $is_reciprocal) {
		$relationship_type = intval($properties_of_map['relationship_type']);
		$index             = 'sections';

		$prefix = $is_reciprocal ? 'reciprocal_' : '';
		...
		$ret = $self->maps->getRelationshipIndex($prefix . $index);
