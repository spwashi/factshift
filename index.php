...
Helper::runHelpers(Helper::PRE_ROUTING);                          # Try to run any validation functions that happen before the router is called. If there is any user IP blocking or whatever, maybe do it here

$router = App::_()->IoC->router;                                # This is the class that will return the route that we want based on the URI
if (!$router || !($router instanceof Router)) exit('Unable to route');
...
App::_()->IoC->session->start();                                # We start the Session here in case there is any autoloading that needs to be done in the matching process
$output = $route->call($http_method);                           # $output is the raw output of the router callback;

Helper::runHelpersReference(Helper::POST_PROCESS, $output);
$output = App::_()->IoC->process_output($output);                       # process this output  via the Output class
Helper::runHelpersReference(Helper::PRE_OUTPUT, $output);            # if there is any secondary processing necessary (e.g. any localization), do it here
echo $output;
